...
def keyholemarkup2x(file, output='df'):
    ...
    if output == 'df' or output == 'dataframe' or output == None:
        ...
        # date = re.search("([0-9]{4}[0-9]{2}[0-9]{2})", file)
        # print(date.group(1))
        # datef = datetime.strptime(date.group(1), '%Y%m%d')

        # df['time'] = [datef for i in range(len(df))]
...
a = keyholemarkup2x("doc.kml",output='shp')